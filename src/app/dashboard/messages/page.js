...
const MessagesPage = () => {
  ...
  return (
    <div className="py-1 min-h-screen message-wrapper dashboard-bg dashboard-color px-3 md:px-12">
      <div className="overflow-x-auto mt-12 mx-2 rounded-2xl">
        <table className="table dashboard-tile w-[150%] lg:w-[100%]">
          <thead>
            ...
            {loading ? (
              ...
              data.map((dt) => (
                <tr key={dt._id}>
                  <td
                    className={
                      dt.status === "unseen" ? "font-bold" : "font-normal"
                    }
                  >
                    {dt.name}
                  </td>
